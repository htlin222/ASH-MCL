...
ggplot(df, aes(x = Outcome, y = Rate, fill = Outcome)) +
  ...
  scale_y_continuous(limits = c(0, 110), breaks = seq(0, 100, 25)) +
  labs(
    title = "BOVen Efficacy in TP53-Mutant MCL",
...
ggplot(df, aes(x = Metric, y = Value, fill = Type)) +
  geom_bar(stat = "identity", width = 0.6) +
  geom_text(
    aes(label = ifelse(Type == "Response", paste0(Value, "%"), Value)),
    vjust = -0.5,
    size = 5,
    fontface = "bold"
  ) +
  scale_fill_manual(
    values = c("Response" = colors$secondary, "Duration" = colors$warning)
  ) +
  scale_y_continuous(limits = c(0, 70)) +
  labs(
    title = "Pirtobrutinib in cBTKi-Refractory MCL",
...
df <- data.frame(
  Regimen = factor(
    rep(c("Evorpacept + R²", "R² Historical"), 4),
    levels = c("Evorpacept + R²", "R² Historical")
  ),
  Metric = factor(
    rep(c("CR", "ORR", "1yr PFS", "1yr OS"), each = 2),
...
ggplot(df, aes(x = Metric, y = Rate, fill = Regimen)) +
  ...
  labs(
    title = "Evorpacept + R² vs Historical R²",
